...
internal class MemoryPackStreamReader : IPackStreamReader
{
    private readonly MessageFormat _format;
    private readonly ReadOnlyMemory<byte> _reader;
    public int Index = 0;

    public MemoryPackStreamReader(
    ...
    public IResponseMessage ReadMessage()
    {
        ...
        {
            return handler.Deserialize(_format.Version, this, signature, 0) as IResponseMessage;
        }
    ...
    public object ReadStruct()
    {
        ...
        if (_format.ReaderStructHandlers.TryGetValue(signature, out var handler))
        {
            var o = handler.Deserialize(_format.Version, this, signature, size);
            return o;
        }
    ...
    private byte[] ReadBytes(int length)
    {
        ...
        var slice = _reader.Span.Slice(Index, length);
    ...
    {
        return _reader.Span[Index++];
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal sbyte NextSByte()
    {
        return (sbyte)_reader.Span[Index++];
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public short NextShort()
    {
        var slice = _reader.Span.Slice(Index, 2);
        Index += 2;
    ...
    public int NextInt()
    {
        var slice = _reader.Span.Slice(Index, 4);
        Index += 4;
    ...
    {
        var slice = _reader.Span.Slice(Index, 8);
        Index += 8;
